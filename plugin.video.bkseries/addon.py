...
def getFilmes(url):
        xbmc.log('[plugin.video.bkseries] L71 - ' + str(url), xbmc.LOGNOTICE)
        link = openURL(url)
...
def getTemporadas(name,url,iconimage):
        ...
        totF = len(tt)
        urlF = url
        xbmc.log('[plugin.video.bkseries] L131 - ' + str(tt), xbmc.LOGNOTICE)
        for i in tt :
...
def getEpisodios(name, url):
        ...
        xbmc.log('[plugin.video.bkseries] L151 - ' + str(url), xbmc.LOGNOTICE)
        ...
        try:
            ...
            for i in e :
                urlF = i.a['href']
                xbmc.log('[plugin.video.bkseries] L158 - ' + str(urlF), xbmc.LOGNOTICE)
                #urlF = urlF.split('v=')[-1]
                urlF = 'https://www.bkseries.com%s' % urlF if urlF.startswith("/campanha") else urlF
                FxID = urlF.split('t=')[-1]
                #urlF = 'https://www.bkseries.com/video/player/token.php?t=%s' % FxID
                urlF = 'https://www.bkseries.com/video/blogger.php?v=&t=%s&d=&key=&f=&w=&w2=&js=&l=' % FxID
...
def player(name,url,iconimage):
        ...
        urlF = url
        xbmc.log('[plugin.video.bkseries] L242 - ' + str(urlF), xbmc.LOGNOTICE)
        link = openURL(urlF)
        soup = BeautifulSoup(link, 'html.parser')
        data = soup('div', {'id':'opcoes-player'})
        btn = data[0]('a',{'class':'nome-server'})
        #xbmc.log('[plugin.video.bkseries] L247 - ' + str(btn), xbmc.LOGNOTICE)
        ...
        #xbmc.log('[plugin.video.bkseries] L264 - ' + str(players), xbmc.LOGNOTICE)
        ...
        xbmc.log('[plugin.video.bkseries] L274 - ' + str(url2Play), xbmc.LOGNOTICE)

        OK = False

        urlVideo = ''
                
        #xbmc.log('[plugin.video.bkseries] L280 - ' + str(urlVideo), xbmc.LOGNOTICE)

        if OK :
            try:
                xbmc.log('[plugin.video.bkseries] L242 - ' + str(urlVideo), xbmc.LOGNOTICE)
        ...
        xbmc.log('[plugin.video.bkseries] L294 - ' + str(url2Play), xbmc.LOGNOTICE)
...
def player_series(name,url,iconimage):
        xbmc.log('[plugin.video.bkseries] L333 - ' + str(url), xbmc.LOGNOTICE)
        OK = True
        mensagemprogresso = xbmcgui.DialogProgress()
        mensagemprogresso.create('FilmesOnlinePlus', 'Obtendo Fontes para ' + name, 'Por favor aguarde...')
        mensagemprogresso.update(0)
        ...
        link = openURL(url)
        soup = BeautifulSoup(link, "html5lib")
        xbmc.log('[plugin.video.bkseries] L343 - ' + str(soup), xbmc.LOGNOTICE)
        conteudo = soup('div',{'class':'geral'})
        filmes = conteudo[0]('div', {'class':'itens'})
        filmes = filmes[0]('a')
        totF = len(filmes)

        xbmc.log('[plugin.video.bkseries] L349 - ' + str(soup), xbmc.LOGNOTICE)

        for s in filmes:
            hname = s.text.encode('utf-8')
            u = s['onclick']
            u = u.replace('window.location.href=','').replace('\'','')
            xbmc.log('[plugin.video.bkseries] L355 - ' + str(u), xbmc.LOGNOTICE)
            hkey = u
            titsT.append(hname)
            idsT.append(hkey)
        ...
        try:
            ...
            xbmc.log('[plugin.video.bkseries] L372 - ' + str(urlVideo), xbmc.LOGNOTICE)
            ...
            elif 'token.php' in urlVideo:
                fxID = urlVideo.split('t=')[-1]
                url2Play = 'https://www.blogger.com/video.g?token=%s' % fxID
                OK = False

            xbmc.log('[plugin.video.bkseries] L394 - ' + str(urlVideo), xbmc.LOGNOTICE)
        ...
        xbmc.log('[plugin.video.bkseries] L410 - ' + str(url2Play), xbmc.LOGNOTICE)
