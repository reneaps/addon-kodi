...
>>>>>>> 0804e9b2825c1451fa279ead98cd8bbeda6c9aec
#####################################################################
...
=======
        ...
        players = conteudo[0]('iframe')
        urlF = players[i]['data-src']
        xbmc.log('[plugin.video.FilmesOnlinePlus] L292 - ' + str(urlF), xbmc.LOGNOTICE)
        ...
        if 'index.html' in urlF :
            ...
            url2Play = 'https://002.yandexcloud.ga/drive/hls/%s/%s.m3u8' % (fxID, fxID)
            OK = False
        elif 'amazondrive.ga' in urlF :
            fxID = urlF.split('v/')[1]
            urlVideo = 'https://amazondrive.ga/api/source/%s' % (fxID)
            data = urllib.urlencode({'r':'','d':'amazondrive.ga'})
            headers = {
                'Referer': urlF,
                'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.142 Safari/537.36',
                'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3',
            ...
            if html:
                ...
                xbmc.log('[plugin.video.FilmesOnlinePlus] L320 - ' + str(url2Play), xbmc.LOGNOTICE)
        ...
        xbmc.log('[plugin.video.FilmesOnlinePlus] L332 - ' + str(urlVideo), xbmc.LOGNOTICE)

        if OK :
            try:
                xbmc.log('[plugin.video.FilmesOnlinePlus] L317 - ' + str(urlVideo), xbmc.LOGNOTICE)
                url2Play = urlresolver.resolve(urlVideo)
            except:
                dialog = xbmcgui.Dialog()
                dialog.ok(" Erro:", " Video removido! ")
        ...
        xbmc.log('[plugin.video.FilmesOnlinePlus] L346 - ' + str(url2Play), xbmc.LOGNOTICE)
