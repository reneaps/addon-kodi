...
def player(name,url,iconimage):
		...
		elif 'fasturl.ga' in urlF:
			fxID = urlF.split('embed-')[-1]
			urlVideo = 'https://vidmoly.to/embed-%s' % (fxID)
			headers = {
				'Referer': urlF,
				'Connection': 'keep-alive',
				'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.142 Safari/537.36',}
		...
		if OK :
			try:
				xbmc.log('[plugin.video.FilmesOnlinePlus] L366 - ' + str(urlVideo), xbmc.LOGNOTICE)
