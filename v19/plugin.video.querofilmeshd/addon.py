...
def getFilmes(name,url,iconimage):
        xbmc.log('[plugin.video.querofilmeshd] L66 - ' + str(url), xbmc.LOGINFO)
        link = openURL(url)
        #link = unicode(link, 'utf-8', 'ignore')
        soup = BeautifulSoup(link, "html.parser")
        try:
...
def getSeries(url):
        xbmc.log('[plugin.video.querofilmeshd] L101- ' + str(url), xbmc.LOGINFO)
        link = openURL(url)
        #link = unicode(link, 'utf-8', 'ignore')
        soup = BeautifulSoup(link, "html.parser")
        try:
...
def getTemporadas(name,url,iconimage):
        xbmc.log('[plugin.video.querofilmeshd] L138 - ' + str(url), xbmc.LOGINFO)
        html = openURL(url)
...
def getEpisodios(name, url):
        xbmc.log('[plugin.video.querofilmeshd] L158 - ' + str(url), xbmc.LOGINFO)
        ...
        for i in itens:
            ...
            xbmc.log('[plugin.video.querofilmeshd] L182 - ' + str(imgF), xbmc.LOGINFO)
...
def pesquisa():
        ...
        if (keyb.isConfirmed()):
                ...
                xbmc.log('[plugin.video.querofilmeshd] L200 - ' + str(url), xbmc.LOGINFO)
                hosts = []
                temp = []
                link = openURL(url)
                soup = BeautifulSoup(link, 'html.parser')
                ...
                for url, titulo, img in hosts:
                    temp = [url, titulo, img]
                    a.append(temp);
                    xbmc.log('[plugin.video.querofilmeshd] L222 - ' + str(a), xbmc.LOGINFO)
...
def doPesquisaSeries():
        ...
        for url2, titulo, img in a:
            xbmc.log('[plugin.video.querofilmeshd] L239 - ' + str(url2), xbmc.LOGINFO)
...
def player(name,url,iconimage):
        xbmc.log('[plugin.video.querofilmeshd] L248 - ' + str(url), xbmc.LOGINFO)
        OK = True
        ...
        try:
                ...
                xbmc.log('[plugin.video.querofilmeshd] L277 - ' + str(urlF), xbmc.LOGINFO)
                ...
                html = r.content
                xbmc.log('[plugin.video.querofilmeshd] L281 - ' + str(html), xbmc.LOGINFO)
                try:
                    soup = BeautifulSoup(html, "html.parser")
                ...
                xbmc.log('[plugin.video.querofilmeshd] L295 - ' + str(url), xbmc.LOGINFO)
                html = openURL(url)
                xbmc.log('[plugin.video.querofilmeshd] L297 - ' + str(html), xbmc.LOGINFO)
                try:
                    ...
                    pass
                xbmc.log('[plugin.video.querofilmeshd] L300 - ' + str(url), xbmc.LOGINFO)
                ...
                xbmc.log('[plugin.video.querofilmeshd] L311 - ' + str(html), xbmc.LOGINFO)
                _html = str(html)
                _html = bytes.fromhex(_html).decode('utf-8')
                b = json.loads(_html)
                xbmc.log('[plugin.video.querofilmeshd] L314 - ' + str(b), xbmc.LOGINFO)
                urlF = b['url']
                ...
                xbmc.log('[plugin.video.querofilmeshd] L341 - ' + str(url2Play), xbmc.LOGINFO)
        except:
            pass

        xbmc.log('[plugin.video.querofilmeshd] L345 - ' + str(urlVideo), xbmc.LOGINFO)
        ...
        if 'video.php' in urlVideo :
                ...
                urlF = soup.iframe["src"]
                urlVideo = urlF
                xbmc.log('[plugin.video.querofilmeshd] L354 - ' + str(urlVideo), xbmc.LOGINFO)
                ...
                url2Play = urlF
                xbmc.log('[plugin.video.querofilmeshd] L361 - ' + str(urlVideo), xbmc.LOGINFO)
                OK = False
                ...
                urlF = soup.source["src"]
                url2Play = urlF
                xbmc.log('[plugin.video.querofilmeshd] L369 - ' + str(urlVideo), xbmc.LOGINFO)
                OK = False

        elif 'index.html' in urlVideo:
                url2Play = urlVideo
                xbmc.log('[plugin.video.querofilmeshd] L374 - ' + str(urlVideo), xbmc.LOGINFO)
                OK = False
                ...
                xbmc.log('[plugin.video.querofilmeshd] L382 - ' + str(match), xbmc.LOGINFO)
                for x in match:
                    idsT.append(x)
                ...
                xbmc.log('[plugin.video.querofilmeshd] L405 - ' + str(idS), xbmc.LOGINFO)
                urlF = 'https://player.querofilmeshd.co//CallPlayer'
                data = urllib.urlencode({'id': idS})
                r = requests.post(url=urlF, data=data, headers=headers)
                html = r.content
                _html = str(html)
                xbmc.log('[plugin.video.querofilmeshd] L411 - ' + str(_html), xbmc.LOGINFO)
                _html = bytes.fromhex(_html).decode('utf-8')
                b = json.loads(_html)
                try:
                        urlF = b['url']
                ...
                xbmc.log('[plugin.video.querofilmeshd] L427 - ' + str(urlVideo), xbmc.LOGINFO)
                ...
                if 'letsupload.co' in urlVideo:
                        ...
                        xbmc.log('[plugin.video.querofilmeshd] L440 - ' + str(url2Play), xbmc.LOGINFO)
                        OK = False
                        ...
                        xbmc.log('[plugin.video.querofilmeshd] L470 - ' + str(urlVideo), xbmc.LOGINFO)
                        OK = True
                        ...
                        r = requests.get(urlF)
                        xbmc.log('[plugin.video.querofilmeshd] L497 - ' + str(r.text), xbmc.LOGINFO)
                        idsT = re.findall('RESOLUTION=.*?\n(.*?)\n',r.text)
                        if len(idsT) > 0:
        ...
        xbmc.log('[plugin.video.querofilmeshd] L515 - ' + str(url2Play), xbmc.LOGINFO)
...
def player_series(name,url,iconimage):
        xbmc.log('[plugin.video.querofilmeshd] L572 - ' + str(url), xbmc.LOGINFO)
        OK = True
        mensagemprogresso = xbmcgui.DialogProgress()
        mensagemprogresso.create('QueroFilmesHD', 'Obtendo Fontes para ' + name + ' Por favor aguarde...')
        mensagemprogresso.update(0)
        ...
        xbmc.log('[plugin.video.querofilmeshd] L588 - ' + str(dados), xbmc.LOGINFO)
        if not dados :
                dialog = xbmcgui.Dialog()
        ...
        try:
                ...
                urlF = base + 'wp-admin/admin-ajax.php'
                xbmc.log('[plugin.video.querofilmeshd] L606 - ' + str(urlF), xbmc.LOGINFO)
                data = urllib.parse.urlencode({'action': 'doo_player_ajax', 'post': dpost, 'nume': dnume, 'type': dtype})
                r = requests.post(url=urlF, data=data, headers=headers)
                html = r.content
                xbmc.log('[plugin.video.querofilmeshd] L610 - ' + str(html), xbmc.LOGINFO)
                try:
                    soup = BeautifulSoup(html, "html.parser")
                    urlF = soup.iframe['src']
                    urlVideo = urlF[0]
                ...
                xbmc.log('[plugin.video.querofilmeshd] L624 - ' + str(url), xbmc.LOGINFO)
                html = openURL(url)
                xbmc.log('[plugin.video.querofilmeshd] L626 - ' + str(html), xbmc.LOGINFO)
                try:
                    url = re.findall(r"<iframe class='metaframe rptss' src='(.*?)' frameborder='0' scrolling='no' allow='autoplay; encrypted-media' allowfullscreen></iframe>", html)[0]
                    html = requests.get(urlF).text
                except:
                    pass
                xbmc.log('[plugin.video.querofilmeshd] L632 - ' + str(url)+'\n'+str(urlF), xbmc.LOGINFO)
                idS = re.findall(r'idS=\s*"(.*?)"', html)[0]
                headers = {'Referer': url,
                           'Accept': '*/*',
                           'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
                ...
                data = urllib.parse.urlencode({'id': idS})
                html = requests.post(url=url, data=data, headers=headers).text
                xbmc.log('[plugin.video.querofilmeshd] L644 - ' + str(url), xbmc.LOGINFO)
                _html = str(html)
                _html = bytes.fromhex(_html).decode('utf-8')
                b = json.loads(_html)
                xbmc.log('[plugin.video.querofilmeshd] L648 - ' + str(b), xbmc.LOGINFO)
                urlF = b['url']
                if '//public' in urlF : urlF = urlF.replace('//public','/public')
                fxID = urlF.split('id=')[1]
                if "&" in fxID: fxID = fxID.split('&')[0]
                ...
                xbmc.log('[plugin.video.querofilmeshd] L654 - ' + str(urlVideo), xbmc.LOGINFO)
        except:
            pass
        try:
            b = json.loads(html)
            ...
            xbmc.log('[plugin.video.querofilmeshd] L661 - ' + str(urlF), xbmc.LOGINFO)
            match = re.findall(r'idS="(.*?)"', html)
            idS = match[0]
            xbmc.log('[plugin.video.querofilmeshd] L664 - ' + str(idS), xbmc.LOGINFO)
            headers = {'Referer': urlF,
                       'Accept': '*/*',
                       'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
            ...
            xbmc.log('[plugin.video.querofilmeshd] L675 - ' + str(r.text), xbmc.LOGINFO)
            _html = str(html)
            _html = bytes.fromhex(_html).decode('utf-8')
            b = json.loads(_html)
            urlF = b['url']
            xbmc.log('[plugin.video.querofilmeshd] L679 - ' + str(urlF), xbmc.LOGINFO)
            urlVideo = urlF
        except:
            pass

        if 'querofilmeshd' in urlVideo:
                r = requests.get(urlVideo)
                html = r.content
                #xbmc.log('[plugin.video.querofilmeshd] L687 - ' + str(html), xbmc.LOGINFO)
                soup = BeautifulSoup(html, 'html.parser')
                try:
                ...
                r = requests.post(url=urlF, data=data, headers=headers)
                xbmc.log('[plugin.video.querofilmeshd] L723 - ' + str(data), xbmc.LOGINFO)
                html = r.content
                _html = str(html)
                _html = bytes.fromhex(_html).decode('utf-8')
                b = json.loads(_html)
                ...
                xbmc.log('[plugin.video.querofilmeshd] L741 - ' + str(urlF), xbmc.LOGINFO)

                idF = urlF.split('id=')[-1]
                urlF = 'https://player.filmesonlinetv.org/hls/%s/%s.m3u8' % (idF,idF)
                xbmc.log('[plugin.video.querofilmeshd] L745 - ' + str(urlF), xbmc.LOGINFO)
                r = requests.get(urlF)
                html = r.text
                html = html.replace('redirect/','')
                xbmc.log('[plugin.video.querofilmeshd] L749 - ' + str(html), xbmc.LOGINFO)

                urlVideo = urlF
                url2Play = urlVideo
                OK = False

                xbmc.log('[plugin.video.querofilmeshd] L755 - ' + str(urlVideo), xbmc.LOGINFO)
                ...
                if 'apiblogger.xyz' in urlVideo :
                        ...
                        url2Play = urlVideo + "|" + urllib.parse.urlencode(headers)
                        OK = False

                elif 'video.php' in urlVideo :
                        fxID = urlVideo.split('=')[1]
                        urlVideo = base64.b64decode(fxID)
                        xbmc.log('[plugin.video.querofilmeshd] L782 - ' + str(urlVideo), xbmc.LOGINFO)
                        OK = True

        xbmc.log('[plugin.video.querofilmeshd] L799 ' + str(urlVideo), xbmc.LOGINFO)
...
def addDir(name, url, mode, iconimage, total=1, pasta=True):
        ...
        xbmc.log('[plugin.video.querofilmeshd] L891 ' + str(u), xbmc.LOGINFO)
